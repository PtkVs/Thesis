...
public class BenchmarkAliceDS {

    private static Logger LOG = LoggerFactory.getLogger(BenchmarkAliceDS.class);
        ...
        for (int run = 0; run < numberOfRuns; run++) {
            ...
            try (CSVReader reader = new CSVReader(new FileReader(csvFilePath))) {
                ...
                for (String[] record : records) {
                    ...
                    LOG.info("Encryption time for record " + count + ": " + encryptionTime + " nanoseconds");
                ...
                LOG.info("Total encryption time for this run: " + totalEncryptionTime + " nanoseconds");
                LOG.info("Average encryption time per record for this run: " + (totalEncryptionTime / processedCount) + " nanoseconds");
            ...
            LOG.info("Execution time for run " + (run + 1) + ": " + runExecutionTime + " nanoseconds");
        }

        // After all runs, log the total and average execution times
        LOG.info("Total execution time for " + numberOfRuns + " runs: " + totalExecutionTime + " nanoseconds");
        LOG.info("Average execution time per run: " + (totalExecutionTime / numberOfRuns) + " nanoseconds");
    }
}
